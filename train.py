...
requested_gpu = None
if "--gpu" in sys.argv:
    gpu_arg_index = sys.argv.index("--gpu")
    if gpu_arg_index + 1 >= len(sys.argv):
        raise ValueError("--gpu requires a device index.")
    requested_gpu = sys.argv[gpu_arg_index + 1]
...
if requested_gpu is not None and requested_gpu != "-1":
    ...
    result = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE).stdout.decode().splitlines()
    used_memory = [int(line.split()[2]) for line in result if line.split()]
    if not used_memory:
        raise RuntimeError("Unable to query GPU memory usage for automatic device selection.")
